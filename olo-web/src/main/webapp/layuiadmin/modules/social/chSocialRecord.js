layui.define(['table','form','laydate','laypage'], function(exports){
	...
	//社保导入记录-表格渲染
	table.render({
		elem: '#LAY-chSocialRecord-manage',
		url: 'index/do.jhtm',
	    ...
	    [
	  	  [
			  ...
			  {field: 'batchUserAccount', title: '操作人账户' ,width:'5%'},
			  ...
			  {field: 'batchMsg', title: '批次消息' ,width:'5%'},
		      ...
		      {title: '操作', width: 150, align: 'center', fixed: 'right', toolbar: '#table-chSocialRecord-manager-operator',width:'15%'}
		  ]
	    ],
	    page: true,
	...
	//监听工具条
	table.on('tool(LAY-chSocialRecord-manage)', function(obj){
	  var data = obj.data;
	  if(obj.event === 'del'){
		  ...
		  layer.open({
			  ...
			  yes: function(index, layero){
				  ...
				  submitID = 'LAY-chSocialRecord-edit-submit'
				  ...
				  iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
					  ...
					  //$.ajax({});
					  table.reload('LAY-chSocialRecord-manage'); //数据刷新
  ...
  //监听搜索
  form.on('submit(LAY-chSocialRecord-search)', function(data){
  	  var field = data.field;
  	  //执行重载
  	  table.reload('LAY-chSocialRecord-manage', {where: field});
  });
  
  //---------表头按钮区域----------
  var active = {
	  batchdel: function(){
	  	var checkStatus = table.checkStatus('LAY-chSocialRecord-manage'),
  ...
  /* 更新页面-------->监听提交 */
  form.on('submit(LAY-chSocialRecord-edit-submit)', function(data){
  	$.ajax({
    	url:'edit/do.jhtm',
        method:'post',
        data:data.field,
        ...
        success:function(data){
			...
			if("200" == code)
			{
				parent.layui.table.reload('LAY-chSocialRecord-manage'); //重载表格
  ...
  form.on('submit(LAY-chSocialRecord-edit-cancel)', function(data){
  	var index = parent.layer.getFrameIndex(window.name);
    parent.layer.close(index);
    return false;
  });
  ...
  /* 添加页面-------->监听提交 */
  form.on('submit(LAY-chSocialRecord-add-submit)', function(data){
  	$.ajax({
    	url:'add/do.jhtm',
        method:'post',
        data:data.field,
  ...
  laydate.render({
  	elem: '#LAY-chSocialRecord_xxx_date'
  });
  ...
  form.verify({
		...
		batchUserAccount: function(value){
			if(value.length > 20)
				return '操作人账户长度不能超过20';
	    },
