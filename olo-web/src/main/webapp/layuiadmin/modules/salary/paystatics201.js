layui.define(['table','form','laydate','layer','element','upload','laypage'], function(exports){
	...
	table.render({
		elem: '#LAY-payslip-statistics201-manage',
		url: 'dostatis.jhtm',
	    ...
	    limit: 10,
            ...
            $.each(res.data, function (index, item) {
            	...
            	if (item.sa_sum_project == "合计") {
	              	$("#LAY-payslip-statistics201-manage").next().find('tbody tr[data-index="' + index +
	...
	//头工具栏事件
  table.on('toolbar(LAY-payslip-statistics201-manage)', function(obj){
	    ...
	    switch(obj.event){
	      ...
	      case 'LAYTABLE_EXPORT_USER':
	    	  ...
	    	  window.location.href=$("#basePath").val()+'/salary/statistics/ex'+'port201.jhtm?ymonth='+$("#ymonth").val();
	    	  break;
	      //自定义头工具栏右侧图标 - 提示
	      case 'LAYTABLE_TIPS':
  ...
  //监听搜索
  form.on('submit(LAY-payslip-statistics201-search)', function(data){
  	  ...
  	  table.reload('LAY-payslip-statistics201-manage', {where: field,cols:xx});
  });
