layui.define(['table','form','laypage'], function(exports){
	...
	table.render({
	    ...
	    [
	  	  [
			  ...
			  {field: 'msgType', title: '警报类型' ,templet:alarmTypeFmt,sort: true,width:'15%'},
			  {field: 'msgCont', title: '警报信息' ,sort: true,width:'35%'},
		      ...
		      {title: '操作', width: 150, align: 'center', fixed: 'right', toolbar: '#table-alarmMsg-manager-operator',width:'20%'}
		  ]
	    ],
	    page: true,
	...
	//监听工具条
	table.on('tool(LAY-alarmMsg-manage)', function(obj){
	  var data = obj.data;
	  if(obj.event === 'del'){
		  ...
		  layer.open({
			  ...
			  yes: function(index, layero){
				  ...
				  submitID = 'LAY-alarmMsg-edit-submit'
				  ...
				  iframeWindow.layui.form.on('submit('+ submitID +')', function(data){
					  ...
					  //$.ajax({});
					  table.reload('LAY-alarmMsg-manage'); //数据刷新
  ...
  //监听搜索
  form.on('submit(LAY-alarmMsg-search)', function(data){
  	  var field = data.field;
  	  //执行重载
  	  table.reload('LAY-alarmMsg-manage', {where: field});
  });
  
  //---------表头按钮区域----------
  var active = {
	  batchdel: function(){
	  	var checkStatus = table.checkStatus('LAY-alarmMsg-manage'),
  ...
  /* 更新页面-------->监听提交 */
  form.on('submit(LAY-alarmMsg-edit-submit)', function(data){
  	$.ajax({
    	url:'edit/do.jhtm',
        method:'post',
        data:data.field,
        ...
        success:function(data){
			...
			if("200" == code)
			{
				parent.layui.table.reload('LAY-alarmMsg-manage'); //重载表格
  ...
  form.on('submit(LAY-alarmMsg-edit-cancel)', function(data){
  	var index = parent.layer.getFrameIndex(window.name);
    parent.layer.close(index);
    return false;
  });
  ...
  /* 添加页面-------->监听提交 */
  form.on('submit(LAY-alarmMsg-add-submit)', function(data){
  	$.ajax({
    	url:'add/do.jhtm',
        method:'post',
        data:data.field,
  ...
  form.verify({
		...
		msgCont: function(value){
			if(null == value || "undefined" == value || "" == value)
				return '警报信息不能为空!';
			if(value.length > 200)
				return '警报信息长度不能超过200';
	    },
