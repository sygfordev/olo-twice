...
layui.config({
  base: '/olo-web/layuiadmin/' //静态资源所在路径
}).extend({
...
layui.use(['jquery', 'authtree', 'form', 'layer'], function(){
  ...
  // 初始化
  $.ajax({
    url: 'userRolePrivEdit/getPriv.jhtm',
    method:'post',
    data:{"userId":$("#userId").val(),"roleId":$("#roleId").val()},
    dataType: 'json',
    success: function(data){
    	if(1==data.code)
    	{
    		layer.msg(data.msg, {offset: '150px',icon: 2,time: 2000});
    		return;
    	}
      ...
      authtree.render('#LAY-auth-tree-index', data.data.trees, {
        ...
        // ,checkSkin: 'primary'
        ,'theme': 'auth-skin-default'
        ,'themePath': '/olo-web/layuiadmin/layui/css/modules/authtree/'
        ,autowidth: true
        ,formFilter: 'authtree-submit-form' // 注意！！！如果不与其他插件render冲突，这个选填
      });
      var authids = authtree.getChecked('#LAY-auth-tree-index');
      $("#selectedPrivIds").val(authids);
  ...
  // 表单提交样例
  form.on('submit(LAY-userRolePrivEdit-submit)', function(obj){
    var authids = authtree.getChecked('#LAY-auth-tree-index');
    console.log('Choosed authids is', authids);
    obj.field.userId = $("#userId").val();
    obj.field.roleId = $("#roleId").val();
    obj.field.systemPrivList = authids;
    $.ajax({
      ...
      data: obj.field,
      success: function(res){
    	  parent.layer.msg(res.message, {offset: '150px',icon: "200"==res.statusCode?1:2,time: 2000});
