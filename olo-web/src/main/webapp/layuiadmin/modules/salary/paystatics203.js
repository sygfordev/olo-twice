layui.define(['table','form','laydate','layer','element','upload','laypage'], function(exports){
	...
	table.render({
		elem: '#LAY-payslip-statistics203-manage',
		url: 'dostatis.jhtm',
	    ...
	    limit: 10,
            ...
            $.each(res.data, function (index, item) {
            	...
            	if (item.sa_sum_project == "合计") {
	              	$("#LAY-payslip-statistics203-manage").next().find('tbody tr[data-index="' + index +
	...
	//头工具栏事件
  table.on('toolbar(LAY-payslip-statistics203-manage)', function(obj){
	    ...
	    switch(obj.event){
	      ...
	      case 'LAYTABLE_EXPORT_USER':
	    	  ...
	    	  window.location.href=$("#basePath").val()+'/salary/statistics/ex'+'port203.jhtm?ymonth='+$("#ymonth").val();
	    	  break;
	      //自定义头工具栏右侧图标 - 提示
	      case 'LAYTABLE_TIPS':
  ...
  //监听搜索
  form.on('submit(LAY-payslip-statistics203-search)', function(data){
  	...
  	var xx =  
  	  ...
  	  table.reload('LAY-payslip-statistics203-manage', {where: field,cols:xx});
  });
